...
void loadData(const char* filename, sdds::Autoshop& as)
{
	...
	while (file)
	{
		...
		try {
			...
			std::cout << "Unrecognized record type: [" << msg << "]" << std::endl;
		}
		catch (int error) {
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [=](const sdds::Vehicle* aVehicle) -> bool {

			if (aVehicle->topSpeed() > 300) {
				return true;
			}
			return false;
		};
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [=](const sdds::Vehicle* aVehicle) -> bool {

			if (aVehicle->condition() == "broken") {
				return true;
			}
			return false;
		};
